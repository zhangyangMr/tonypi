...
def face_recognition(frame, known_faces, known_labels, conn, threshold=0.6):
    ...
    if result != "Unknown" and result != "No face detected":
        ...
        img_dir = Path(f"./data/face_img/{today_dir}")
        ...
        text_to_wav_file(host=tts_url, text=play_text)
    else:
...
def call_camera(tts_url):
    ...
    # 插入签到人员信息
    insert_csv_data(conn, "./dependence/features.csv")
        ...
        if img is not None:
            ...
            for (x, y, w, h) in faces:
                ...
                if is_centered:
                    ...
                    text_to_wav_file(text=f"请将脸向{direction}移动到屏幕中央", host=tts_url)
...
if __name__ == '__main__':
    ...
    tts_url = config.get('tts')['bobfintech']['tts_base_url']
